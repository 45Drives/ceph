...
void *ObjBencher::status_printer(void *_bencher)
{
    ...
    while (!data.done)
    {
        std::cout << "data is not still done!!!!!"<< std::endl;
...
// FKH encryption benchmarking added aio_bench

int ObjBencher::aio_bench_enc(
    int operation, int secondsToRun,
    int concurrentios,
    uint64_t op_size, uint64_t object_size,
    unsigned max_objects,
